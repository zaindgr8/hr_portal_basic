  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12 animate-in fade-in duration-500">
      <button 
        ...
        className="flex items-center gap-1.5 text-[13px] font-medium text-[#444444] hover:text-[#032D60] transition-colors group"
      >
        <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-0.5 transition-transform" /> Back to Candidates
      </button>

      {/* Header Profile Card */}
      <div className="bg-white rounded-3xl border border-[#DDDBDA]/60 shadow-sm p-8 relative overflow-hidden">
        {/* Subtle background pattern/gradient */}
        <div className="absolute top-0 right-0 -mt-20 -mr-20 w-64 h-64 bg-[#F4F6F9] rounded-full blur-3xl opacity-50 pointer-events-none" />
        
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 relative z-10">
          <div className="flex items-start gap-5">
            <div className="w-16 h-16 rounded-2xl bg-[#EAEAEA] border border-[#DDDBDA]/60 flex items-center justify-center text-[#032D60] font-semibold text-xl shrink-0">
              SJ
            </div>
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-semibold tracking-tight text-[#032D60]">{candidate.name}</h1>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-3">
                <span className="text-[14px] text-[#181818]">{candidate.role}</span>
                <span className="w-1 h-1 rounded-full bg-[#C9C9C9]" />
                ...
                <span className="w-1 h-1 rounded-full bg-[#C9C9C9]" />
                <span className="text-[13px] text-[#444444]">Starts: {candidate.startDate}</span>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <button 
              ...
              className="px-4 py-2 bg-white border border-[#DDDBDA] text-[#181818] rounded-xl text-[13px] font-medium hover:bg-[#F4F6F9] transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
            ...
            <button 
              ...
              className="px-4 py-2 bg-[#0176D3] text-white rounded-xl text-[13px] font-medium hover:bg-[#014486] transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
        ...
        <div className="mt-8 pt-6 border-t border-[#EAEAEA] relative z-10">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[13px] font-medium text-[#032D60]">Evidence Completeness</h3>
            <span className="text-[13px] font-medium text-[#444444]">4 / 6 items complete</span>
          </div>
          <div className="w-full bg-[#EAEAEA] rounded-full h-2 overflow-hidden">
            <div className="h-full rounded-full bg-[#0176D3]" style={{ width: '66%' }} />
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium bg-[#EAEAEA] text-[#181818] border border-[#DDDBDA]/60">
      ...
      {/* AI Flag Section */}
      <div className="bg-indigo-50/30 border border-indigo-100 rounded-3xl p-6 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-[#DDDBDA]/60 shadow-sm p-8">
            <h2 className="text-[16px] font-semibold tracking-tight text-[#032D60] mb-8">Clearance Workflow</h2>
            
            <div className="relative">
              <div className="absolute top-2 bottom-2 left-[11px] w-px bg-[#EAEAEA]" />
                  ...
                  <li key={stage.id} className="flex gap-5 group">
                    <div className="relative bg-white z-10 mt-0.5 shrink-0">
                      {stage.status === 'completed' ? (
                        <div className="w-6 h-6 rounded-full bg-[#0176D3] flex items-center justify-center shadow-sm">
                          <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />
                        </div>
                      ) : stage.status === 'current' ? (
                        <div className="w-6 h-6 rounded-full border-2 border-[#0176D3] flex items-center justify-center bg-white shadow-sm">
                          <div className="w-2 h-2 rounded-full bg-[#0176D3]" />
                        </div>
                      ) : (
                        <div className="w-6 h-6 rounded-full border-2 border-[#DDDBDA] bg-white" />
                      )}
                    </div>
                    <div className={`flex-1 ${stage.status === 'upcoming' ? 'opacity-50' : ''}`}>
                      <div className="flex items-center justify-between">
                        <h3 className={`text-[14px] font-medium ${stage.status === 'current' ? 'text-[#032D60]' : 'text-[#181818]'}`}>
                          {stage.name}
                        </h3>
                        <span className="text-[12px] text-[#747474]">{stage.date}</span>
                      </div>
                      
                      {stage.status === 'current' && (
                        <div className="mt-4 p-5 bg-[#F4F6F9]/50 border border-[#DDDBDA]/60 rounded-2xl">
                          <p className="text-[13px] text-[#181818] mb-4">
                            Upload evidence of FCA Register verification to proceed.
                          </p>
                          <div 
                            onClick={() => showToast('File upload dialog opened')}
                            className="border border-dashed border-[#C9C9C9] rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-[#F4F6F9] transition-colors cursor-pointer bg-white"
                          >
                            <div className="w-10 h-10 rounded-full bg-[#EAEAEA] flex items-center justify-center mb-3">
                              <UploadCloud className="w-5 h-5 text-[#444444]" />
                            </div>
                            <p className="text-[13px] font-medium text-[#032D60]">Click to upload or drag and drop</p>
                            <p className="text-[12px] text-[#444444] mt-1">PDF, PNG, JPG up to 10MB</p>
                          </div>
                        </div>
                      )}
                      ...
                      {stage.status === 'completed' && (
                        <div className="mt-2.5 flex items-center gap-2">
                          <div 
                            ...
                            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#F4F6F9] rounded-lg text-[12px] font-medium text-[#181818] border border-[#DDDBDA]/60 hover:bg-[#EAEAEA] transition-colors cursor-pointer"
                          >
                            <FileText className="w-3.5 h-3.5 text-[#747474]" />
          ...
          <div className="bg-white rounded-3xl border border-[#DDDBDA]/60 shadow-sm p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[16px] font-semibold tracking-tight text-[#032D60]">Fit & Proper Assessment</h2>
              <button 
                onClick={() => showToast('Opening F&P Assessment form')}
                className="text-[13px] font-medium text-[#444444] hover:text-[#032D60] transition-colors"
              >
                Edit Assessment
              </button>
            </div>
            
            <div className="border border-[#DDDBDA]/60 rounded-xl overflow-hidden">
              <table className="w-full text-left text-[13px]">
                <thead className="bg-[#F4F6F9]/50 border-b border-[#DDDBDA]/60 text-[#444444]">
                  <tr>
                ...
                <tbody className="divide-y divide-[#EAEAEA]">
                  {[
                    ...
                    <tr key={i} className="bg-white">
                      <td className="px-4 py-4 font-medium text-[#032D60]">{row.section}</td>
                      ...
                      <td className="px-4 py-4 text-[#444444]">{row.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl border border-[#DDDBDA]/60 shadow-sm p-6">
            <h2 className="text-[11px] font-semibold text-[#747474] mb-5 uppercase tracking-widest">Audit Trail</h2>
            <div className="space-y-5">
              {[
                ...
                <div key={i} className="relative pl-4 border-l-2 border-[#EAEAEA]">
                  <div className="absolute w-2 h-2 rounded-full bg-[#DDDBDA] -left-[5px] top-1.5 border-2 border-white" />
                  <p className="text-[13px] font-medium text-[#032D60]">{log.action}</p>
                  <p className="text-[#747474] text-[11px] mt-1">by {log.user} • {log.time}</p>
                </div>
              ))}
            </div>
            <button 
              onClick={() => showToast('Full audit history opened')}
              className="w-full mt-6 py-2 text-[12px] font-medium text-[#444444] hover:text-[#032D60] transition-colors bg-[#F4F6F9] rounded-lg"
            >
              View full history
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-[#DDDBDA]/60 shadow-sm p-6">
            <h2 className="text-[11px] font-semibold text-[#747474] mb-5 uppercase tracking-widest">IT Provisioning</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between text-[13px]">
                <span className="text-[#181818] font-medium">Trading Systems</span>
              ...
              <div className="flex items-center justify-between text-[13px]">
                <span className="text-[#181818] font-medium">Client Orders</span>
                <span className="px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wider bg-amber-100/50 text-amber-700 border border-amber-200/50">BLOCK</span>
              </div>
              <div className="flex items-center justify-between text-[13px]">
                <span className="text-[#181818] font-medium">Advisory Tools</span>
                <span className="px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wider bg-amber-100/50 text-amber-700 border border-amber-200/50">BLOCK</span>
              </div>
            </div>
            <div className="mt-5 pt-4 border-t border-[#EAEAEA]">
              <p className="text-[12px] text-[#444444] leading-relaxed">
                Access will automatically update to <span className="font-medium text-[#181818]">ALLOW</span> once clearance is issued.
              </p>
            </div>
          </div>
        </div>
